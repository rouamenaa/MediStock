...
public class RabbitMqConfig {
    ...
    @Bean
    public Queue stockLowAlertQueue(
            @Value("${medistock.rabbitmq.stock-low-alert-queue:stock.low.alert.queue}") String queueName) {
        return QueueBuilder.durable(queueName).build();
    }
    ...
    public Binding lowStockAlertBinding(
            Queue stockLowAlertQueue,
            DirectExchange medistockExchange,
            @Value("${medistock.rabbitmq.stock-low-routing-key:stock.low}") String routingKey) {
        return BindingBuilder.bind(stockLowAlertQueue).to(medistockExchange).with(routingKey);
    }
}
